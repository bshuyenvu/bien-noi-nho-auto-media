...
try{
  ...
  const first=enqueuePublish({ownerId:owner,renderJobId:a.render,draftId:a.draft,platform:'youtube',title:'Giá vàng hôm nay biến động mạnh',dryRun:false});
  ...
  const activeDup=add(4,'Giá vàng cập nhật trong ngày',body1.replace('hôm nay','trong ngày'),'https://news.example.test/gold?utm_source=queue','video-active-dup');let activeBlocked=false;try{enqueuePublish({ownerId:owner,renderJobId:activeDup.render,draftId:activeDup.draft,platform:'youtube',title:'Giá vàng cập nhật trong ngày',dryRun:false})}catch(e){activeBlocked=String(e).includes('Content Safety')}if(!activeBlocked)throw new Error('duplicate active PUBLIC job was not blocked before first publish completed');
  run("UPDATE publish_jobs SET status='published',published_at=?,updated_at=? WHERE id=?",new Date(Date.now()-120000).toISOString(),now,first.id);

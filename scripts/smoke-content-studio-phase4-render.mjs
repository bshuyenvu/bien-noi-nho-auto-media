...
const {db,all}=await import('../dist/storage/db.js');
...
const localMedia=media.readySceneMediaForRender(ownerId,project.id);assert.equal(localMedia.length,1);assert.equal(localMedia[0].sceneIndex,0);assert.equal(localMedia[0].path,ready.outputPath);
const batch=await generation.enqueuePipelineGeneration({ownerId,projectId:project.id,outputId:'short-9x16',localMedia});
const short=batch.outputs.find(x=>x.outputId==='short-9x16');assert.ok(short?.renderJobId);
const renderRow=all('SELECT payload_json FROM render_jobs WHERE id=? AND owner_id=?',short.renderJobId,ownerId)[0];assert.ok(renderRow?.payload_json);
const renderPayload=JSON.parse(renderRow.payload_json);assert.equal(renderPayload.localMedia?.[0]?.path,ready.outputPath);assert.equal(renderPayload.localMedia?.[0]?.sceneIndex,0);
console.log('Content Studio Phase 4 local keyframe + provenance Docker smoke OK',JSON.stringify({jobId:ready.id,sceneIndex:ready.sceneIndex,provider:ready.providerId,output:ready.outputPath,artifacts:artifacts.length,renderJobId:short.renderJobId,renderLocalMedia:renderPayload.localMedia.length}));

...
if(!meta||meta.phase!=='7.6')throw new Error('Phase 7 metadata missing');
...
console.log(edited.provider==='rules'?'Phase 7 live LLM smoke DEGRADED-SAFE':'Phase 7 live LLM smoke OK',JSON.stringify({provider:edited.provider,model:status.providers.find((x:any)=>x.id===edited.provider)?.model||'configured',hookStrategy:meta.hookStudio?.selected?.strategy,quality:meta.quality?.score,duration:edited.estimatedSeconds,storyId:Boolean(meta.storyId)}));

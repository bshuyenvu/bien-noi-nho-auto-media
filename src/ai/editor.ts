...
function hook(title:string,body:string){const h=headline(title,body),s=title+' '+body;if(SENSITIVE.test(s))return'Thông tin đang được nhiều người quan tâm: '+h+'.';if(/\b(cảnh báo|nguy cơ|bão|lũ|cháy)\b/i.test(s))return /^(cảnh báo|đáng chú ý)[:：]/i.test(h)?h+'.':'Cảnh báo đáng chú ý: '+h+'.';return h+'. Đây là những điểm người xem cần biết.'}

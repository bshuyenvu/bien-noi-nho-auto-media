...
export function planUsage(access:AccessContext){
 ...
 const defaults=PLAN_DEFAULTS[access.plan as AccountPlan],dailyLimit=account?.daily_limit??defaults.daily,totalLimit=account?.total_limit??defaults.total;
 ...
 return{plan:access.plan,label:defaults.label,dailyLimit,totalLimit,today,total,remaining:totalLimit!=null?Math.max(0,totalLimit-total):dailyLimit!=null?Math.max(0,dailyLimit-today):null,activeKeys,supportZalo:SUPPORT_ZALO};
}

...
export function evaluateOriginality(headline:string,script:string,sources:string[]):OriginalityResult{
  ...
  if(headlineSimilarity>.72)reasons.push('Tiêu đề mới quá giống nguồn tham khảo');
...
export function assertOriginalEditorial(headline:string,script:string,sources:string[]){
  const result=evaluateOriginality(headline,script,sources);

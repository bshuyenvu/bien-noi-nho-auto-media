...
export async function importArticleFromUrl(url:string):Promise<ImportedArticle>{
  ...
  const response=await fetch(url,{headers:{'user-agent':'Mozilla/5.0 (compatible; BienNoiNhoAutoMedia/2.0; +https://media.huyenvu.cloud)'},signal:AbortSignal.timeout(15000)});
  ...
  return {title:title.slice(0,180),body,sourceName:sourceName.slice(0,120)||undefined,sourceUrl:url,imageUrl:imageUrls[0],imageUrls};
}

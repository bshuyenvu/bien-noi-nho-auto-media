...
type Row={draft_id:string;owner_id:string;url:string;source_name?:string;source_url?:string;media_kind:'image'|'video';created_at:string;rights?:MediaRights;creator?:string;license_url?:string;rights_verified?:number};
db.exec(`CREATE TABLE IF NOT EXISTS draft_media_provenance(
 ...
 rights_verified INTEGER NOT NULL DEFAULT 0, PRIMARY KEY(draft_id,url)
);CREATE INDEX IF NOT EXISTS idx_draft_media_provenance_owner ON draft_media_provenance(owner_id,draft_id);`);
for(const sql of ['ALTER TABLE draft_media_provenance ADD COLUMN rights TEXT','ALTER TABLE draft_media_provenance ADD COLUMN creator TEXT','ALTER TABLE draft_media_provenance ADD COLUMN license_url TEXT','ALTER TABLE draft_media_provenance ADD COLUMN rights_verified INTEGER NOT NULL DEFAULT 0'])try{db.exec(sql)}catch{}
...
export function saveDraftMediaProvenance(draftId:string,ownerId:string,items:DraftMediaProvenance[]){
 ...
 for(const item of items.slice(0,30)){
  ...
  run('INSERT OR REPLACE INTO draft_media_provenance(draft_id,owner_id,url,source_name,source_url,media_kind,created_at,rights,creator,license_url,rights_verified) VALUES(?,?,?,?,?,?,?,?,?,?,?)',draftId,ownerId,url,clean(item.sourceName,160)||null,clean(item.sourceUrl,2000)||null,item.kind==='video'?'video':'image',now,item.rights||'unverified',clean(item.creator,160)||null,clean(item.licenseUrl,2000)||null,item.rightsVerified===true?1:0);
 }
 return getDraftMediaProvenance(draftId,ownerId)
}
export function getDraftMediaProvenance(draftId:string,ownerId:string):DraftMediaProvenance[]{return all<Row>('SELECT * FROM draft_media_provenance WHERE draft_id=? AND owner_id=? ORDER BY created_at,url',draftId,ownerId).map(x=>({url:x.url,sourceName:x.source_name||undefined,sourceUrl:x.source_url||undefined,kind:x.media_kind==='video'?'video':'image',rights:x.rights||'unverified',creator:x.creator||undefined,licenseUrl:x.license_url||undefined,rightsVerified:x.rights_verified===1}))}

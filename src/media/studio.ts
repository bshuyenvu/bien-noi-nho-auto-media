...
export interface StudioCandidate extends MediaCandidate{id:string;selected:boolean;source:'manual'|'article'}
export async function analyzeMediaStudio(input:{sourceUrl?:string;imageUrl?:string;imageUrls?:string[]}){
 ...
 const candidates:StudioCandidate[]=result.all.sort((a,b)=>b.score-a.score).map((x,i)=>({...x,id:`media-${i+1}`,selected:selected.has(x.path),source:x.manual?'manual':'article'}));
 await Promise.allSettled(downloaded.map(x=>rm(x.path,{force:true})));
 return{candidates,errors,summary:{found:urls.length,downloaded:downloaded.length,accepted:result.selected.length,rejected:result.rejected.length}};
}

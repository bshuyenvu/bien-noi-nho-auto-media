...
export async function prepareAutoNews(input:{ownerId?:string;url:string;length:ScriptLength;format:'breaking'|'latest'|'standard';fallback?:{title:string;summary?:string;sourceName?:string;imageUrl?:string}}){
 ...
 return{stage:'review' as const,article:{sourceUrl:article.sourceUrl,sourceName:article.sourceName,originalTitle:article.title,language:article.language},intelligence:{id:intelligence.id,language:intelligence.language,translationStatus:intelligence.translationStatus,sourceScore:intelligence.sourceScore,authorityScore:intelligence.authorityScore,freshnessScore:intelligence.freshnessScore,corroborationCount:intelligence.corroborationCount,provider:intelligence.provider,facts:intelligence.facts,uncertainties:intelligence.uncertainties,warnings:intelligence.warnings,readyForEditorial:intelligence.readyForEditorial},research:{sources:research.sources.map(x=>({name:x.name,url:x.url,title:x.title})),count:research.sources.length},edited,media:{images:chosen.map(x=>({url:x.url,score:x.score,width:x.width,height:x.height,metadata:x.metadata})),summary:studio.summary},scenes,voice,reviewRequired:true};
}

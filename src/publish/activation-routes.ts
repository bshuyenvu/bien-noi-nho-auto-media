...
activationWizardRouter.post('/admin/activation-wizard/verify-remote-canary',async(_req,res)=>{try{const x=await verifyRemoteCanary(owner(res),actor(res).id),v=x.remoteVerification;audit(res,'activation.remote-canary-verify','Xác minh Remote Canary trực tiếp bằng YouTube Data API',{videoId:v.videoId,channelId:v.channelId,privacyStatus:v.privacyStatus,uploadStatus:v.uploadStatus,processingStatus:v.processingStatus,checkedAt:v.checkedAt});return res.json(x)}catch(e){return res.status(409).json({error:e instanceof Error?e.message:String(e)})}});
activationWizardRouter.post('/admin/activation-wizard/approve-public',async(req,res)=>{const p=Confirm.safeParse(req.body||{});if(!p.success)return res.status(400).json({error:'Cần confirmation'});try{const x=await approvePublicActivation(owner(res),actor(res).id,p.data.confirmation);audit(res,'activation.public-approve','Phê duyệt mức YouTube PUBLIC',{maxPrivacy:x.state.maxPrivacy,unlistedTestVideoId:x.state.unlistedTestVideoId,remoteCanaryVerifiedAt:x.state.remoteCanaryVerifiedAt});return res.json(x)}catch(e){return res.status(409).json({error:e instanceof Error?e.message:String(e)})}});

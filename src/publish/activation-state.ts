...
export type PublicRolloutStatus='idle'|'canary_queued'|'watching'|'completed'|'failed';
export interface ProductionActivationState{
  ...
  publicRolloutStatus?:PublicRolloutStatus;
  publicCanaryJobId?:string;
  publicCanaryVideoId?:string;
  publicCanaryVerifiedAt?:string;
  publicCanaryChannelId?:string;
  publicCanaryTitle?:string;
  publicWatchStartedAt?:string;
  publicWatchEndsAt?:string;
  publicRolloutCompletedAt?:string;
  publicRolloutFailureReason?:string;
  abortedAt?:string;
  updatedAt?:string;
  updatedBy?:string;
}
...
export function productionActivationState(ownerId:string):ProductionActivationState{return read(activationKey(ownerId),{ownerId,status:'idle',armed:false,maxPrivacy:'private',publicRolloutStatus:'idle'} as ProductionActivationState)}
...
export function clearPublicRollout(){return{publicRolloutStatus:'idle' as PublicRolloutStatus,publicCanaryJobId:undefined,publicCanaryVideoId:undefined,publicCanaryVerifiedAt:undefined,publicCanaryChannelId:undefined,publicCanaryTitle:undefined,publicWatchStartedAt:undefined,publicWatchEndsAt:undefined,publicRolloutCompletedAt:undefined,publicRolloutFailureReason:undefined}}
export function abortProductionActivation(ownerId:string,actor:string){const now=new Date().toISOString();return updateProductionActivation(ownerId,{status:'aborted',armed:false,maxPrivacy:'private',abortedAt:now,sessionId:undefined,armedAt:undefined,unlistedAuthorizedAt:undefined,unlistedVerifiedAt:undefined,unlistedTestVideoId:undefined,remoteCanaryVerifiedAt:undefined,remoteCanaryVideoId:undefined,remoteCanaryChannelId:undefined,remoteCanaryTitle:undefined,remoteCanaryPrivacyStatus:undefined,remoteCanaryUploadStatus:undefined,remoteCanaryProcessingStatus:undefined,publicApprovedAt:undefined,...clearPublicRollout()},actor)}
export function productionPublishGuard(ownerId:string,input:{deploymentTest?:boolean;publicCanary?:boolean;privacy?:ActivationPrivacy}={}){
  ...
  if(privacy==='public'){
    if(!state.publicApprovedAt)return{allowed:false,reason:'PUBLIC chưa được phê duyệt trong Activation Wizard',killSwitch:kill,state,privacy};
    if(input.publicCanary){
      if(!['idle','canary_queued'].includes(String(state.publicRolloutStatus||'idle')))return{allowed:false,reason:`Public Canary không được phép khi rollout=${state.publicRolloutStatus}`,killSwitch:kill,state,privacy};
      return{allowed:true,reason:'PUBLIC approved • chỉ Public Canary được phép trước rollout completion',killSwitch:kill,state,privacy};
    }
    if(state.publicRolloutStatus!=='completed'||!state.publicRolloutCompletedAt)return{allowed:false,reason:'PUBLIC thường bị khóa cho đến khi Public Canary và Post-Publish Watch hoàn tất',killSwitch:kill,state,privacy};
  }
  return{allowed:true,reason:`Activation armed • max ${state.maxPrivacy}${privacy==='public'?' • public rollout completed':''}`,killSwitch:kill,state,privacy};
}

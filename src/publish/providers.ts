...
export interface PublisherProvider{
  platform:PublishPlatform;
  validateCredential(credential?:PublishCredential):void;
  publish(input:PublishRequest):Promise<PublishResult>;
}

function dryRunResult(platform:PublishPlatform,job:PublishJob):PublishResult{
  return{remoteId:`dry-${platform}-${job.id}`,remoteUrl:undefined,publishedAt:new Date().toISOString(),dryRun:true};
}
...
class YouTubeProvider implements PublisherProvider{
  ...
  async publish(input:PublishRequest){if(input.job.dryRun)return dryRunResult(this.platform,input.job);this.validateCredential(input.credential);throw new Error('YouTube live publishing chưa được bật; dùng dry-run cho đến khi OAuth được kết nối')}
}
class FacebookProvider implements PublisherProvider{
  ...
  async publish(input:PublishRequest){if(input.job.dryRun)return dryRunResult(this.platform,input.job);this.validateCredential(input.credential);throw new Error('Facebook live publishing chưa được bật; dùng dry-run cho đến khi Page OAuth được kết nối')}
}

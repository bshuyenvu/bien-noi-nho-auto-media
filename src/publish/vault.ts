...
function publicMetadata(row:Row){
  ...
  try{
    ...
    const scopesOk=YOUTUBE_REQUIRED_SCOPES.every(x=>scopes.includes(x));
    return{channelId:channelId||undefined,channelTitle:channelTitle||undefined,scopesOk,refreshTokenReady:Boolean(secret.refreshToken),liveReadyCached:Boolean(secret.refreshToken&&channelId&&scopesOk),verifiedAt:secret.verifiedAt?String(secret.verifiedAt):undefined};
  }catch{return{liveReadyCached:false}}
}

...
function isNormalPublic(job:PublishJob){return !job.dryRun&&job.platform==='youtube'&&!job.deploymentTest&&!job.publicCanary&&(job.publishPrivacy||envYouTubePrivacy())==='public'}
function quarantineInterrupted(row:Row,reason='server_restart_during_publish'){
  ...
  run("UPDATE publish_jobs SET status='needs_reconcile',error=?,reconcile_reason=?,reconcile_at=?,reconciled_at=NULL,reconciled_by=NULL,reconcile_note=NULL,updated_at=? WHERE id=? AND status='publishing'",message,reason,now,now,row.id);if(Boolean(row.public_canary))failPublicCanary(row.owner_id,`${reason}: ${message}`);if(row.publish_privacy==='public'&&!Boolean(row.public_canary))recordPublicRampFailure(row.owner_id,message,{needsReconcile:true});
}
...
export async function processPublishJob(job:PublishJob){
  ...
  if(!job.dryRun&&job.platform==='youtube'){
    ...
    if(isNormalPublic(job)){
      try{assertPublicRampAllowed(job.ownerId,{targetAt:new Date().toISOString(),excludeJobId:job.id})}
      catch(e){
        if(e instanceof PublicRampGuardError&&e.kind==='rate'&&e.nextAllowedAt){const now=new Date().toISOString();run("UPDATE publish_jobs SET status='scheduled',scheduled_at=?,error=?,updated_at=? WHERE id=? AND owner_id=?",e.nextAllowedAt,e.message.slice(0,700),now,job.id,job.ownerId);return{deferred:true,nextAllowedAt:e.nextAllowedAt}}
        if(e instanceof PublicRampGuardError&&e.kind==='circuit'){const now=new Date().toISOString();run("UPDATE publish_jobs SET status='failed',error=?,updated_at=? WHERE id=? AND owner_id=?",e.message.slice(0,700),now,job.id,job.ownerId)}
        throw e;
      }
    }
  }
  ...
  try{
    ...
    if(isNormalPublic(job))recordPublicRampSuccess(job.ownerId);
    processed++;lastSuccessAt=completedAt;lastError=undefined;lastErrorAt=undefined;return result;
  }catch(e){
    const message=e instanceof Error?e.message:String(e),now=new Date().toISOString(),needsReconcile=e instanceof YouTubeUploadNeedsReconcileError||Boolean((e as any)?.needsReconcile);
    if(needsReconcile){
    ...
    if(job.publicCanary)failPublicCanary(job.ownerId,message);else if(isNormalPublic(job))recordPublicRampFailure(job.ownerId,message,{needsReconcile});
...
export async function runPublishWorkerOnce(){
  ...
  try{for(const job of dueJobs()){try{const result=await processPublishJob(job);if(!(result as any)?.deferred)count++}catch{}}return{ok:true,busy:false,processed:count}}finally{running=false}
}

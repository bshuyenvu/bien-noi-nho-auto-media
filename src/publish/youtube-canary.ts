...
export async function verifyYouTubeUnlistedCanary(credential:PublishCredential|undefined,videoId:string):Promise<YouTubeCanaryVerification>{
  ...
  const base={ok:false,readyForPromotion:false,videoId:id,expectedChannelId,checkedAt,reasons:[] as string[]};
  ...
  try{
    ...
    const channelId=String(item.snippet?.channelId||''),title=String(item.snippet?.title||''),privacyStatus=String(item.status?.privacyStatus||''),uploadStatus=String(item.status?.uploadStatus||''),processingStatus=String(item.processingDetails?.processingStatus||'');
    ...
    if(privacyStatus!=='unlisted')reasons.push(`privacyStatus phải là unlisted, hiện tại là ${privacyStatus||'unknown'}`);
    ...
    return{ok:true,readyForPromotion:reasons.length===0,videoId:id,expectedChannelId,channelId,title,privacyStatus,uploadStatus,processingStatus,failureReason,rejectionReason,processingFailureReason,checkedAt,reasons};

...
type DraftRow={id:string;owner_id?:string;title:string;body:string;source_url?:string;source_name?:string;image_url?:string;format:string};
...
function draftRow(draftId:string,ownerId?:string){return ownerId?all<DraftRow>('SELECT id,owner_id,title,body,source_url,source_name,image_url,format FROM drafts WHERE id=? AND owner_id=? LIMIT 1',draftId,ownerId)[0]:all<DraftRow>('SELECT id,owner_id,title,body,source_url,source_name,image_url,format FROM drafts WHERE id=? LIMIT 1',draftId)[0]}
...
export function setReview(draftId:string,input:{status:ReviewStatus;locks:ReviewLocks},meta:{ownerId?:string;actor?:string;reason?:string}={}){
  const draft=draftRow(draftId,meta.ownerId);if(!draft)throw new Error('Draft not found for review');const owner=draft.owner_id||meta.ownerId||'legacy-admin',currentHash=draftHash(draft),now=new Date().toISOString(),actor=String(meta.actor||owner).slice(0,200),allLocked=Object.values(input.locks).every(Boolean),approved=(input.status==='approved'||input.status==='ready')&&allLocked,status:ReviewStatus=(input.status==='approved'||input.status==='ready')&&!allLocked?'needs_review':input.status;

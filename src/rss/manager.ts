...
export interface RssSource { id:string; ownerId:string; name:string; url:string; createdAt:string; lastScanAt?:string; lastError?:string; locked:boolean; managed:boolean; }
export interface RssItem { id:string; ownerId:string; sourceId:string; sourceName:string; title:string; link:string; summary?:string; imageUrl?:string; publishedAt?:string; discoveredAt:string; }

type SourceRow={id:string;owner_id?:string;name:string;url:string;created_at:string;last_scan_at?:string;last_error?:string;locked?:number;managed?:number};
type ItemRow={id:string;owner_id?:string;source_id:string;source_name:string;title:string;link:string;summary?:string;image_url?:string;published_at?:string;discovered_at:string};
const mapSource=(r:SourceRow):RssSource=>({id:r.id,ownerId:r.owner_id||'legacy-admin',name:r.name,url:r.url,createdAt:r.created_at,lastScanAt:r.last_scan_at||undefined,lastError:r.last_error||undefined,locked:Boolean(r.locked),managed:Boolean(r.managed)});
const mapItem=(r:ItemRow):RssItem=>({id:r.id,ownerId:r.owner_id||'legacy-admin',sourceId:r.source_id,sourceName:r.source_name,title:r.title,link:r.link,summary:r.summary||undefined,imageUrl:r.image_url||undefined,publishedAt:r.published_at||undefined,discoveredAt:r.discovered_at});
export const rssSources:RssSource[]=all<SourceRow>('SELECT * FROM rss_sources_tenant ORDER BY created_at DESC').map(mapSource);
export const rssItems:RssItem[]=all<ItemRow>('SELECT * FROM rss_items_tenant ORDER BY discovered_at DESC LIMIT 300').map(mapItem);
const seen=new Set(rssItems.map(i=>i.ownerId+'|'+i.link));
...
export function updateRssSource(id:string,input:{name?:string;url?:string}){const source=rssSources.find(s=>s.id===id);if(!source)return undefined;const nextUrl=directRssUrl(input.url?.trim()||source.url);const duplicate=rssSources.find(s=>s.id!==id&&s.ownerId===source.ownerId&&s.url===nextUrl);if(duplicate)throw new Error('RSS URL đã tồn tại');const nextName=(input.name?.trim()||new URL(nextUrl).hostname.replace(/^www\./,'')).slice(0,120);source.name=nextName;source.url=nextUrl;source.lastError=undefined;run('UPDATE rss_sources_tenant SET name=?,url=?,last_error=NULL WHERE id=?',source.name,source.url,source.id);for(const item of rssItems.filter(x=>x.sourceId===id)){item.sourceName=source.name;run('UPDATE rss_items_tenant SET source_name=? WHERE id=?',source.name,item.id)}return source;}

export function setRssSourceLock(id:string,locked:boolean){const source=rssSources.find(s=>s.id===id);if(!source)return undefined;source.locked=locked;if(locked)source.managed=false;run('UPDATE rss_sources_tenant SET locked=?,managed=? WHERE id=?',source.locked?1:0,source.managed?1:0,id);return source}

export function deleteRssSource(id:string,deleteItems=true){const index=rssSources.findIndex(s=>s.id===id);if(index<0)return false;if(deleteItems){const removed=rssItems.filter(x=>x.sourceId===id);for(const item of removed)seen.delete(item.ownerId+'|'+item.link);for(let i=rssItems.length-1;i>=0;i--)if(rssItems[i].sourceId===id)rssItems.splice(i,1);run('DELETE FROM rss_items_tenant WHERE source_id=?',id)}rssSources.splice(index,1);run('DELETE FROM rss_sources_tenant WHERE id=?',id);return true;}

export function deleteRssItem(id:string){const index=rssItems.findIndex(x=>x.id===id);if(index<0)return false;seen.delete(rssItems[index].ownerId+'|'+rssItems[index].link);rssItems.splice(index,1);run('DELETE FROM rss_items_tenant WHERE id=?',id);return true;}

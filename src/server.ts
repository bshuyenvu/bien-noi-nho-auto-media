...
app.get('/health',(_q,r)=>r.json({ok:true,service:'bien-noi-nho-auto-media',version:'1.6.0',mode:'self-hosted',storage:'sqlite',ai:process.env.AI_API_URL&&process.env.AI_API_KEY&&process.env.AI_MODEL?'llm+local-fallback':'local-editor',templates:['classic','breaking','clean'],voices:VOICE_CATALOG.length,motion:'ken-burns'}));
...
app.post('/api/drafts/:id/render',(q,r)=>{const d=drafts.find(x=>x.id===q.params.id);if(!d)return r.status(404).json({error:'Draft not found'});const p=z.object({voice:z.string().default('vi-male'),voiceRate:z.enum(['-20%','-10%','+0%','+10%','+20%']).default('+0%'),template:z.enum(['classic','breaking','clean']).optional()}).safeParse(q.body||{});if(!p.success||!isVoiceId(p.data.voice))return r.status(400).json({error:'Tùy chọn render không hợp lệ'});d.status='rendering';saveDraft(d);const job=enqueueRender({draftId:d.id,text:`${d.title}. ${d.body}`,headline:d.title,source:d.sourceName,breaking:d.format==='breaking',voice:p.data.voice,voiceRate:p.data.voiceRate,imageUrl:d.imageUrl,template:p.data.template});return r.status(202).json(job)});
app.get('/',(_q,r)=>r.sendFile('app.html',{root:'public'}));app.listen(PORT,'0.0.0.0',()=>console.log(`Auto Media V1.6 Motion + Multilingual TTS running on :${PORT}`));

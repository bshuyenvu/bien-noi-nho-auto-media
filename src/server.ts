...
app.post('/api/drafts',(q,r)=>{const p=NewsDraft.safeParse(q.body);if(!p.success)return r.status(400).json({error:p.error.flatten()});const d:Draft={...p.data,id:crypto.randomUUID(),ownerId:accessOf(r).accountId,status:'draft',createdAt:new Date().toISOString()};drafts.unshift(d);saveDraft(d);ensureQueueItem(d.id,d.title);return r.status(201).json({...d,review:getReview(d.id,d.ownerId)})});
...
app.get('/api/drafts/:id/review',(q,r)=>{const d=ownDraft(r,q.params.id);if(!d)return r.status(404).json({error:'Draft not found'});const review=getReview(d.id,d.ownerId);return r.json({review,history:listReviewEvents(d.id,d.ownerId,20)})});
app.post('/api/drafts/:id/review',(q,r)=>{const d=ownDraft(r,q.params.id);if(!d)return r.status(404).json({error:'Draft not found'});const p=ReviewPayload.safeParse(q.body||{});if(!p.success)return r.status(400).json({error:'Dữ liệu duyệt không hợp lệ'});const review=setReview(d.id,p.data,{ownerId:d.ownerId,actor:reviewActor(r)});if(review.approvalCurrent){d.status='approved';setQueueStatus(d.id,'approved')}else{if(d.status==='approved')d.status='draft';setQueueStatus(d.id,'waiting_review')}saveDraft(d);return r.json({draft:d,review,history:listReviewEvents(d.id,d.ownerId,20)})});
app.post('/api/drafts/:id/approve',(q,r)=>{const d=ownDraft(r,q.params.id);if(!d)return r.status(404).json({error:'Draft not found'});const locks={script:true,media:true,voice:true,scenes:true};const review=setReview(d.id,{status:'approved',locks},{ownerId:d.ownerId,actor:reviewActor(r),reason:'quick_approve'});d.status='approved';saveDraft(d);setQueueStatus(d.id,'approved');return r.json({...d,review,history:listReviewEvents(d.id,d.ownerId,20)})});

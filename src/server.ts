...
const PORT=Number(process.env.PORT||8787),API_KEY=process.env.RENDER_API_KEY;
...
const drafts:Draft[]=all<DraftRow>('SELECT * FROM drafts ORDER BY created_at DESC').map(r=>({id:r.id,title:r.title,body:r.body,sourceUrl:r.source_url||undefined,sourceName:r.source_name||undefined,imageUrl:r.image_url||undefined,format:r.format,status:r.status,createdAt:r.created_at}));
...
app.post('/api/rss-sources',(q,r)=>{const p=z.object({url:z.string().url(),name:z.string().max(120).optional()}).safeParse(q.body);if(!p.success)return r.status(400).json({error:'RSS URL không hợp lệ'});try{return r.status(201).json(addRssSource({...p.data,locked:true,managed:false}))}catch(e){return r.status(422).json({error:e instanceof Error?e.message:String(e)})}});
...
app.get('/api/drafts',(_q,r)=>{syncDraftStatuses(drafts);return r.json(drafts)});
app.post('/api/drafts',(q,r)=>{const p=NewsDraft.safeParse(q.body);if(!p.success)return r.status(400).json({error:p.error.flatten()});const d:Draft={...p.data,id:crypto.randomUUID(),status:'draft',createdAt:new Date().toISOString()};drafts.unshift(d);saveDraft(d);ensureQueueItem(d.id,d.title);return r.status(201).json({...d,review:getReview(d.id)})});

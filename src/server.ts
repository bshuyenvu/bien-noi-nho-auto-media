...
app.post('/api/drafts/:id/render',(q,r)=>{const d=drafts.find(x=>x.id===q.params.id);if(!d)return r.status(404).json({error:'Draft not found'});const p=z.object({voice:z.string().default('vi-male'),voiceRate:z.enum(['-20%','-10%','+0%','+10%','+20%']).default('+0%'),template:z.enum(['classic','breaking','clean']).optional(),motion:z.enum(['off','light','medium','strong']).default('light'),tickerMode:z.enum(['off','headline','custom']).default('headline'),tickerText:z.string().max(500).optional(),tickerSpeed:z.number().min(35).max(180).default(85)}).safeParse(q.body||{});if(!p.success||!isVoiceId(p.data.voice))return r.status(400).json({error:'Tùy chọn render không hợp lệ'});d.status='rendering';saveDraft(d);const job=enqueueRender({draftId:d.id,text:`${d.title}. ${d.body}`,headline:d.title,source:d.sourceName,breaking:d.format==='breaking',voice:p.data.voice,voiceRate:p.data.voiceRate,imageUrl:d.imageUrl,template:p.data.template,motion:p.data.motion,tickerMode:p.data.tickerMode,tickerText:p.data.tickerText,tickerSpeed:p.data.tickerSpeed});return r.status(202).json(job)});
app.get('/',(_q,r)=>r.sendFile('app.html',{root:'public'}));app.listen(PORT,'0.0.0.0',()=>console.log(`Auto Media V1.8 Ticker Studio running on :${PORT}`));

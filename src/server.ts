...
app.get('/health',(_q,r)=>r.json({ok:true,service:'bien-noi-nho-auto-media',version:'1.4.0',mode:'self-hosted',storage:'sqlite',ai:process.env.AI_API_URL&&process.env.AI_API_KEY&&process.env.AI_MODEL?'llm+local-fallback':'local-editor'}));
...
app.post('/api/drafts/:id/render',(q,r)=>{const d=drafts.find(x=>x.id===q.params.id);if(!d)return r.status(404).json({error:'Draft not found'});d.status='rendering';saveDraft(d);const job=enqueueRender({draftId:d.id,text:`${d.title}. ${d.body}`,headline:d.title,source:d.sourceName,breaking:d.format==='breaking',voice:q.body?.voice==='female'?'female':'male',imageUrl:d.imageUrl});return r.status(202).json(job)});
app.get('/',(_q,r)=>r.sendFile('app.html',{root:'public'}));app.listen(PORT,'0.0.0.0',()=>console.log(`Auto Media V1.4 self-hosted + SQLite + AI Editor running on :${PORT}`));

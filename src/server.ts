...
import { VOICE_CATALOG, isVoiceId } from './tts/edge.js';
...
app.get('/health',(_q,r)=>r.json({ok:true,service:'bien-noi-nho-auto-media',version:'2.0.0',mode:'self-hosted',storage:'sqlite',ai:process.env.AI_API_URL&&process.env.AI_API_KEY&&process.env.AI_MODEL?'llm+local-fallback':'local-editor',templates:['classic','breaking','clean'],voices:VOICE_CATALOG.length,motion:['off','light','medium','strong'],ticker:['off','headline','custom'],media:{images:10,transition:'fade',autoCollect:true}}));
...
app.get('/api/templates',(_q,r)=>r.json([{id:'classic',name:'Bản tin',description:'Bố cục bản tin chuẩn.'},{id:'breaking',name:'Tin nóng',description:'Header đỏ mạnh cho tin nóng.'},{id:'clean',name:'Tin mới',description:'Theme xanh hiện đại, ưu tiên hình ảnh.'}]));
app.get('/api/voices',(_q,r)=>r.json(VOICE_CATALOG.map(v=>({id:v.id,name:v.name,language:v.language,locale:v.locale,gender:v.gender}))));
...
app.post('/api/drafts/:id/render',(q,r)=>{const d=drafts.find(x=>x.id===q.params.id);if(!d)return r.status(404).json({error:'Draft not found'});const p=z.object({voice:z.string().default('vi-male'),voiceRate:z.enum(['-20%','-10%','+0%','+10%','+20%']).default('+0%'),template:z.enum(['classic','breaking','clean']).optional(),motion:z.enum(['off','light','medium','strong']).default('light'),tickerMode:z.enum(['off','headline','custom']).default('headline'),tickerText:z.string().max(500).optional(),tickerSpeed:z.number().min(35).max(180).default(85),imageUrls:z.array(z.string().url()).max(9).default([]),autoCollectImages:z.boolean().default(true)}).safeParse(q.body||{});if(!p.success||!isVoiceId(p.data.voice))return r.status(400).json({error:'Tùy chọn render không hợp lệ'});d.status='rendering';saveDraft(d);const job=enqueueRender({draftId:d.id,text:`${d.title}. ${d.body}`,headline:d.title,source:d.sourceName,sourceUrl:d.sourceUrl,breaking:d.format==='breaking',voice:p.data.voice,voiceRate:p.data.voiceRate,imageUrl:d.imageUrl,imageUrls:p.data.imageUrls,autoCollectImages:p.data.autoCollectImages,template:p.data.template,motion:p.data.motion,tickerMode:p.data.tickerMode,tickerText:p.data.tickerText,tickerSpeed:p.data.tickerSpeed});return r.status(202).json(job)});
app.get('/',(_q,r)=>r.sendFile('app.html',{root:'public'}));app.listen(PORT,'0.0.0.0',()=>console.log(`Auto Media V2.0 Auto Image Collector running on :${PORT}`));

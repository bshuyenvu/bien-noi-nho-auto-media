...
try{db.exec('ALTER TABLE render_jobs ADD COLUMN started_at TEXT')}catch{}
try{db.exec('ALTER TABLE render_jobs ADD COLUMN completed_at TEXT')}catch{}
try{db.exec('ALTER TABLE publish_jobs ADD COLUMN deployment_test INTEGER NOT NULL DEFAULT 0')}catch{}
try{db.exec('ALTER TABLE publish_jobs ADD COLUMN started_at TEXT')}catch{}
try{db.exec('ALTER TABLE publish_jobs ADD COLUMN completed_at TEXT')}catch{}
...
try{db.exec("UPDATE render_jobs SET completed_at=updated_at WHERE completed_at IS NULL AND status IN ('ready','failed')")}catch{}
try{db.exec("UPDATE publish_jobs SET completed_at=updated_at WHERE completed_at IS NULL AND status IN ('published','failed','cancelled')")}catch{}
...
try{db.exec('CREATE INDEX IF NOT EXISTS idx_render_jobs_owner_completed ON render_jobs(owner_id,completed_at)')}catch{}
try{db.exec('CREATE INDEX IF NOT EXISTS idx_publish_jobs_owner_completed ON publish_jobs(owner_id,completed_at)')}catch{}
db.exec(`
CREATE TRIGGER IF NOT EXISTS trg_render_metrics_retry_reset AFTER UPDATE OF status ON render_jobs
WHEN NEW.status='queued' AND OLD.status='failed'
BEGIN UPDATE render_jobs SET started_at=NULL,completed_at=NULL WHERE id=NEW.id; END;
CREATE TRIGGER IF NOT EXISTS trg_render_metrics_start AFTER UPDATE OF status ON render_jobs
WHEN NEW.status='rendering' AND OLD.status!='rendering' AND NEW.started_at IS NULL
BEGIN UPDATE render_jobs SET started_at=strftime('%Y-%m-%dT%H:%M:%fZ','now'),completed_at=NULL WHERE id=NEW.id; END;
CREATE TRIGGER IF NOT EXISTS trg_render_metrics_complete AFTER UPDATE OF status ON render_jobs
WHEN NEW.status IN ('ready','failed') AND OLD.status!=NEW.status
BEGIN UPDATE render_jobs SET completed_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id=NEW.id; END;
CREATE TRIGGER IF NOT EXISTS trg_publish_metrics_retry_reset AFTER UPDATE OF status ON publish_jobs
WHEN NEW.status='pending' AND OLD.status='failed'
BEGIN UPDATE publish_jobs SET started_at=NULL,completed_at=NULL WHERE id=NEW.id; END;
CREATE TRIGGER IF NOT EXISTS trg_publish_metrics_start AFTER UPDATE OF status ON publish_jobs
WHEN NEW.status='publishing' AND OLD.status!='publishing'
BEGIN UPDATE publish_jobs SET started_at=strftime('%Y-%m-%dT%H:%M:%fZ','now'),completed_at=NULL WHERE id=NEW.id; END;
CREATE TRIGGER IF NOT EXISTS trg_publish_metrics_complete AFTER UPDATE OF status ON publish_jobs
WHEN NEW.status IN ('published','failed','cancelled') AND OLD.status!=NEW.status
BEGIN UPDATE publish_jobs SET completed_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id=NEW.id; END;
`);

...
productionMonitorRouter.post('/admin/monitoring/incidents/:id/ack',requireAdmin,(req,res)=>{const ownerId=accessOf(res).accountId;try{return res.json(acknowledgeIncident(req.params.id,ownerId,ownerId,req.body?.note?String(req.body.note):undefined))}catch(e){return res.status(404).json({error:e instanceof Error?e.message:String(e)})}});
productionMonitorRouter.post('/admin/monitoring/incidents/:id/silence',requireAdmin,(req,res)=>{const ownerId=accessOf(res).accountId;try{return res.json(silenceIncident(req.params.id,ownerId,ownerId,minutesOf(req.body?.minutes),req.body?.reason?String(req.body.reason):undefined))}catch(e){return res.status(404).json({error:e instanceof Error?e.message:String(e)})}});
productionMonitorRouter.delete('/admin/monitoring/incidents/:id/silence',requireAdmin,(req,res)=>{const ownerId=accessOf(res).accountId;try{return res.json(unsilenceIncident(req.params.id,ownerId))}catch(e){return res.status(404).json({error:e instanceof Error?e.message:String(e)})}});

...
function reconcileIncident(ownerId:string,key:string,component:string,severity:MonitorSeverity,message:string,metadata:Record<string,unknown>){
  ...
  if(severity==='green'){
    if(open)run('UPDATE system_incidents SET resolved_at=?,last_seen_at=? WHERE id=?',now,now,open.id);
    return;
  }
  const json=JSON.stringify(metadata);
  if(open)run('UPDATE system_incidents SET severity=?,message=?,metadata_json=?,last_seen_at=? WHERE id=?',severity,message,json,now,open.id);
  else run('INSERT INTO system_incidents(id,owner_id,incident_key,component,severity,message,metadata_json,opened_at,last_seen_at,resolved_at) VALUES(?,?,?,?,?,?,?,?,?,NULL)',incidentId(ownerId,key),ownerId,key,component,severity,message,json,now,now);
}
...
export async function productionMonitorSnapshot(ownerId?:string){
  const [render,storage]=await Promise.all([renderWorkerStatus(),storageSnapshot().catch(()=>null)]),resources=resourceSnapshot(),publish=publishWorkerStatus();
  const warnMemoryMb=envNumber('MONITOR_WARN_AVAILABLE_MB',768),warnLoadPerCpu=envNumber('MONITOR_WARN_LOAD_PER_CPU',0.70),loadPerCpu=resources.cpuLoad1m/Math.max(1,resources.cpuCount);
  const memorySeverity:MonitorSeverity=!resources.memoryOk?'red':resources.availableMemoryMb<warnMemoryMb?'yellow':'green';
  ...
  const renderSeverity:MonitorSeverity=render.watchdog?.stalled?'red':render.shuttingDown||render.paused?'yellow':'green';
  const publishErrorAge=ageMs(publish.lastErrorAt),publishSeverity:MonitorSeverity=!publish.started?'yellow':publish.lastError&&publishErrorAge<10*60_000?'red':publish.lastError&&publishErrorAge<60*60_000?'yellow':'green';
  ...
  const components={
    memory:{severity:memorySeverity,message:memorySeverity==='green'?'RAM ổn định':`RAM khả dụng ${resources.availableMemoryMb} MB`,availableMb:resources.availableMemoryMb,warnMb:warnMemoryMb},
    ...
    render:{severity:renderSeverity,message:render.watchdog?.stalled?`Render worker STALLED tại ${render.watchdog.stage}`:render.paused?'Render queue đang PAUSED':'Render worker hoạt động',busy:render.busy,pending:render.pending,currentJobId:render.currentJobId,watchdog:render.watchdog,lastActivityAt:render.lastActivityAt},
    publish:{severity:publishSeverity,message:publishSeverity==='green'?'Publish worker hoạt động':publish.lastError||'Publish worker chưa khởi động',...publish},
  ...
  return{overall,checkedAt:new Date().toISOString(),components,render,publish,deployment,monitor:{started:Boolean(timer),intervalMs:envNumber('MONITOR_INTERVAL_MS',30000),lastCycleAt,lastCycleError}};
}

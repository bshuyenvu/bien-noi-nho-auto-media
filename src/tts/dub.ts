...
export async function dubVideoFile(input:{videoPath:string;text:string;voice:VoiceId;style:VoiceStyle;rate:string;outputPath:string;ownerId?:string}){
...
export async function dubDirectVideo(input:{url:string;text:string;voice:VoiceId;style:VoiceStyle;rate:string;rightsConfirmed:boolean;ownerId?:string}){
 ...
 try{videoPath=await downloadRemoteVideo(input.url,`${base}-source`);return{id,...await dubVideoFile({videoPath,text:input.text,voice:input.voice,style:input.style,rate:input.rate,outputPath:`${base}.mp4`,ownerId:input.ownerId})}}finally{if(videoPath)await rm(videoPath,{force:true}).catch(()=>undefined)}}

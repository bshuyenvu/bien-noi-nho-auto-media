...
export async function synthesizeSrtTimeline(input:{srt:string;voice:VoiceId;style:VoiceStyle;rate:string;ownerId?:string}){
 ...
 try{
  for(let i=0;i<analysis.cues.length;i++){const cue=analysis.cues[i],path=`output/${id}-cue-${String(i+1).padStart(3,'0')}.mp3`;cueFiles.push(path);
   await generateSpeech({text:cue.text,audioPath:path,voice:input.voice,style:input.style,rate:input.rate,ownerId:input.ownerId});const actual=await audioDuration(path),slot=cue.durationMs/1000,ratio=actual/slot;

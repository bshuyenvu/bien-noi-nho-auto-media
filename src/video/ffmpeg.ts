...
export async function renderNewsVideo(opts:{audioPath:string;srtPath?:string;outputPath:string;duration?:number;headline?:string;source?:string;breaking?:boolean;imagePath?:string;template?:VideoTemplate;motion?:MotionLevel}){
 ...
 const overlay=templateOverlay(template,label,headlineFile,sourceFile,opts.srtPath);const mediaY=template==='breaking'?650:630,mediaH=template==='breaking'?740:760;
 if(opts.imagePath){const filter=`${imageMotion(opts.imagePath,mediaH,motionLevel)};[0:v][img]overlay=55:${mediaY}[base];[base]${overlay}[v]`;await run('ffmpeg',['-y','-f','lavfi','-i',`color=c=0x04111f:s=1080x1920:r=30:d=${duration}`,'-loop','1','-i',opts.imagePath,'-i',opts.audioPath,'-filter_complex',filter,'-map','[v]','-map','2:a:0','-c:v','libx264','-preset','veryfast','-crf','23','-pix_fmt','yuv420p','-c:a','aac','-b:a','128k','-shortest','-movflags','+faststart',opts.outputPath]);}
 else{const filters=`drawbox=x=55:y=${mediaY}:w=970:h=${mediaH}:color=0x172033:t=fill,drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf:text='ẢNH / VIDEO MINH HỌA':fontcolor=0x94a3b8:fontsize=38:x=(w-text_w)/2:y=990,${overlay}`;await run('ffmpeg',['-y','-f','lavfi','-i',`color=c=0x04111f:s=1080x1920:r=30:d=${duration}`,'-i',opts.audioPath,'-vf',filters,'-map','0:v:0','-map','1:a:0','-c:v','libx264','-preset','veryfast','-crf','23','-pix_fmt','yuv420p','-c:a','aac','-b:a','128k','-shortest','-movflags','+faststart',opts.outputPath]);}

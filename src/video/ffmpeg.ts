...
export async function renderNewsVideo(opts:{audioPath:string;srtPath?:string;outputPath:string;duration?:number;headline?:string;source?:string;breaking?:boolean;imagePath?:string}){
 ...
 const overlay=[
  ...
  `drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:text='${headline}':fontcolor=white:fontsize=48:x=85:y=305:fix_bounds=true`,
 ...
 if(opts.imagePath){
  const filter=`[1:v]scale=970:760:force_original_aspect_ratio=increase,crop=970:760[img];[0:v][img]overlay=55:630[base];[base]${overlay}[v]`;
  ...
  const filters=`drawbox=x=55:y=630:w=970:h=760:color=0x172033:t=fill,drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf:text='ẢNH / VIDEO MINH HỌA':fontcolor=0x94a3b8:fontsize=38:x=(w-text_w)/2:y=990,${overlay}`;

...
function wrapHeadline(input:string,max=28,maxLines=4){
 ...
 for(;index<words.length;index++){
  const word=words[index];const next=line?`${line} ${word}`:word;
  if(next.length<=max){line=next;continue;}
  if(line)lines.push(line);
  line=word;
  if(lines.length>=maxLines-1){index++;break;}
 }
 if(line&&lines.length<maxLines)lines.push(line);
 if(index<words.length&&lines.length)lines[lines.length-1]=lines[lines.length-1].replace(/[.,;:!?]*$/,'')+'…';
 return lines.join('\n');
}
...
function templateOverlay(template:VideoTemplate,label:string,headlineFile:string,sourceFile?:string,srtPath?:string){
 ...
 if(template==='clean')return [
  `drawbox=x=0:y=0:w=1080:h=178:color=0x071a33:t=fill`,
  `drawbox=x=0:y=0:w=280:h=178:color=0x075bc7:t=fill`,
  `drawbox=x=0:y=174:w=1080:h=4:color=0x22d3ee:t=fill`,
  `drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:text='TIN MỚI':fontcolor=white:fontsize=42:x=58:y=68`,
  `drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:text='BIỂN & NỖI NHỚ':fontcolor=white:fontsize=34:x=335:y=72`,
  `drawbox=x=38:y=225:w=1004:h=360:color=0x0b1628@0.93:t=fill`,
  `drawbox=x=38:y=584:w=1004:h=3:color=0x1da1f2:t=fill`,
  `drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:${headlineText}:fontcolor=white:fontsize=46:line_spacing=14:x=68:y=278:fix_bounds=true`,
  `drawbox=x=55:y=1405:w=970:h=78:color=0x061426@0.94:t=fill`,
  `drawbox=x=55:y=1405:w=170:h=78:color=0xc81e1e:t=fill`,
  `drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:text='NGUỒN':fontcolor=white:fontsize=25:x=82:y=1430`,
  sourceFile?`drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf:textfile='${escPath(sourceFile)}':reload=0:fontcolor=white:fontsize=22:x=245:y=1433`:'null',
  `drawbox=x=55:y=1498:w=970:h=52:color=0x071a33@0.96:t=fill`,
  `drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf:text='BIỂN & NỖI NHỚ • TIN MỚI':fontcolor=0x7dd3fc:fontsize=20:x=78:y=1515`,
  subtitle
 ].join(',');
 if(template==='breaking')return [
  ...
  `drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:${headlineText}:fontcolor=white:fontsize=50:line_spacing=14:x=78:y=285:fix_bounds=true`,
  source,subtitle
 ].join(',');
 return [
  ...
  `drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:${headlineText}:fontcolor=white:fontsize=48:line_spacing=12:x=85:y=300:fix_bounds=true`,
  source,subtitle
 ].join(',');
}
...
export async function renderNewsVideo(opts:{audioPath:string;srtPath?:string;outputPath:string;duration?:number;headline?:string;source?:string;breaking?:boolean;imagePath?:string;template?:VideoTemplate}){
 ...
 // Long enough for 90s scripts; -shortest ends the video with the narration.
 const duration=opts.duration??180,label=opts.breaking?'TIN NÓNG':'TIN MỚI',template=opts.template??(opts.breaking?'breaking':'classic');
 ...
 if(opts.source?.trim()){
  ...
  await writeFile(sourceFile,opts.source.trim(),'utf8');
 }
 ...
 if(opts.imagePath){
  // Gentle Ken Burns motion: slow center zoom, capped at 6%.
  const motion=`[1:v]scale=1200:${mediaH+160}:force_original_aspect_ratio=increase,crop=1200:${mediaH+160},zoompan=z='min(max(zoom,pzoom)+0.00045,1.06)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s=970x${mediaH}:fps=30[img]`;
  const filter=`${motion};[0:v][img]overlay=55:${mediaY}[base];[base]${overlay}[v]`;
  await run('ffmpeg',['-y','-f','lavfi','-i',`color=c=0x04111f:s=1080x1920:r=30:d=${duration}`,'-loop','1','-i',opts.imagePath,'-i',opts.audioPath,'-filter_complex',filter,'-map','[v]','-map','2:a:0','-c:v','libx264','-preset','veryfast','-crf','23','-pix_fmt','yuv420p','-c:a','aac','-b:a','128k','-shortest','-movflags','+faststart',opts.outputPath]);
 }else{

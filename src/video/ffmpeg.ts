...
function templateOverlay(template:VideoTemplate,label:string,headlineFile:string,sourceFile?:string,srtPath?:string){
 ...
 if(template==='clean')return [
  ...
  `drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:${headlineText}:fontcolor=white:fontsize=46:line_spacing=14:x=82:y=285:fix_bounds=true`
 ].join(',');

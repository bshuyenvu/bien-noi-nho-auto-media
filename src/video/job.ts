...
export function enqueueRender(input:{draftId:string;text:string;headline:string;source?:string;breaking?:boolean;voice?:VoiceId;voiceRate?:string;imageUrl?:string;imageUrls?:string[];template?:VideoTemplate;motion?:MotionLevel;tickerMode?:TickerMode;tickerText?:string;tickerSpeed?:number}){
 ...
 void(async()=>{try{
  ...
  const urls=[input.imageUrl,...(input.imageUrls||[])].filter((x):x is string=>Boolean(x)).filter((x,i,a)=>a.indexOf(x)===i).slice(0,10);
  ...
  for(let i=0;i<urls.length;i++){
   ...
   job.progress=Math.min(35,15+Math.round(((i+1)/Math.max(1,urls.length))*20));save(job);
  }

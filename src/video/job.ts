...
function mediaCredit(url?:string,items?:DraftMediaProvenance[]){if(!url)return undefined;const exact=items?.find(x=>x.url===url),named=provenanceCredit(exact).replace(/^(?:Ảnh|Video):\s*/i,'').trim();if(named)return named;try{return new URL(url).hostname.replace(/^www\./,'').slice(0,80)}catch{return undefined}}

...
export function enqueueRender(input:{draftId:string;text:string;headline:string;source?:string;sourceUrl?:string;breaking?:boolean;voice?:VoiceId;voiceRate?:string;imageUrl?:string;imageUrls?:string[];autoCollectImages?:boolean;template?:VideoTemplate;motion?:MotionLevel;tickerMode?:TickerMode;tickerText?:string;tickerSpeed?:number}){
 ...
 void(async()=>{try{
  job.status='rendering';job.progress=10;save(job);const base=`output/${job.id}`;
  let discovered:string[]=[];
  if(input.autoCollectImages!==false&&input.sourceUrl){
   try{const article=await importArticleFromUrl(input.sourceUrl);discovered=article.imageUrls||[];job.progress=18;save(job);}catch(e){console.warn('Auto image collection skipped:',e)}
  }
  const urls=[input.imageUrl,...(input.imageUrls||[]),...discovered].filter((x):x is string=>Boolean(x)).filter((x,i,a)=>a.indexOf(x)===i).slice(0,10);
  const imagePaths:string[]=[];
  for(let i=0;i<urls.length;i++){
   try{imagePaths.push(await downloadRemoteImage(urls[i],`${base}-image-${i+1}`));}
   catch(e){console.warn(`Image ${i+1} download skipped:`,e)}
   job.progress=Math.min(38,20+Math.round(((i+1)/Math.max(1,urls.length))*18));save(job);
  }
  await generateSpeech({text:input.text,voice:input.voice??'vi-male',rate:input.voiceRate??'+0%',audioPath:`${base}.mp3`,srtPath:`${base}.srt`});job.progress=60;save(job);
  job.output=await renderNewsVideo({audioPath:`${base}.mp3`,srtPath:`${base}.srt`,outputPath:`${base}.mp4`,headline:input.headline,source:input.source,breaking:input.breaking,imagePaths,template:input.template,motion:input.motion,tickerMode:input.tickerMode,tickerText:input.tickerText,tickerSpeed:input.tickerSpeed});
  job.progress=100;job.status='ready';save(job);

...
import { cancelActiveFfmpeg, renderNewsVideo, type VideoTemplate, type MotionLevel, type TickerMode, type VideoScene, type RenderMediaItem } from './ffmpeg.js';
import { renderStudioLandscapeVideo } from './ffmpeg-landscape.js';
import { mediaDurationSeconds } from './duration.js';
...
import { mediaCredit as provenanceCredit, type DraftMediaProvenance } from '../media/provenance.js';
import { copyrightSafeMode,mediaRightsAllowed } from '../compliance/copyright.js';
import { createSceneVisualCard } from './scene-card.js';

export type RenderStatus = 'queued' | 'rendering' | 'ready' | 'failed';
function mediaCredit(url?:string,items?:DraftMediaProvenance[]){if(!url)return undefined;const exact=items?.find(x=>x.url===url),named=provenanceCredit(exact).replace(/^(?:Ảnh|Video):\s*/i,'').trim();if(named)return named;if(copyrightSafeMode())return undefined;try{return new URL(url).hostname.replace(/^www\./,'').slice(0,80)}catch{return undefined}}
export interface EnqueueRenderInput {
  ...
  autoCollectImages?:boolean; smartScenes?:boolean; shotCraft?:boolean; scenes?:VideoScene[]; template?:VideoTemplate; motion?:MotionLevel;
  tickerMode?:TickerMode; tickerText?:string; tickerSpeed?:number; channelName?:string; mediaProvenance?:DraftMediaProvenance[];
  localMedia?:RenderMediaItem[];
  renderMode?:'vertical'|'landscape'|'audio';
}
...
export async function deleteRenderJob(id:string){const i=renderJobs.findIndex(j=>j.id===id),row=i<0?all<JobRow>('SELECT * FROM render_jobs WHERE id=?',id)[0]:undefined;if(i<0&&!row)return false;const job=i>=0?renderJobs.splice(i,1)[0]:{id:row!.id,draftId:row!.draft_id,ownerId:row!.owner_id||'legacy-admin',status:row!.status,progress:row!.progress,createdAt:row!.created_at,updatedAt:row!.updated_at||row!.created_at,attempts:Number(row!.attempts||0),maxAttempts:Number(row!.max_attempts||DEFAULT_MAX_ATTEMPTS)} as RenderJob;renderInputs.delete(id);const p=pendingRenders.findIndex(x=>x.job.id===id);if(p>=0)pendingRenders.splice(p,1);run('DELETE FROM render_jobs WHERE id=?',id);await cleanupArtifacts(job.id);return true}
...
async function processRender(job:RenderJob,input:EnqueueRenderInput){try{await waitWhilePaused();job.attempts++;job.nextAttemptAt=undefined;currentJobId=job.id;job.status='rendering';job.progress=2;job.error=undefined;checkpoint(job,'resource-check',input);await waitForRenderResources();job.progress=7;checkpoint(job,'resources-ready',input);await ensureStorageForRender([job.id]);job.progress=10;checkpoint(job,'starting',input);
const base=`output/${job.id}`;const strictCopyright=copyrightSafeMode();let discovered:string[]=[],publishedAt=input.publishedAt;if(!strictCopyright&&input.autoCollectImages!==false&&input.sourceUrl){try{checkpoint(job,'collecting-media');const article=await importArticleFromUrl(input.sourceUrl);discovered=article.imageUrls||[];publishedAt=publishedAt||article.publishedAt;job.progress=18;checkpoint(job,'media-collected')}catch(e){console.warn('Auto image collection skipped:',e)}}
const provenanceByUrl=new Map((input.mediaProvenance||[]).map(x=>[x.url,x] as const));const permitted=new Set((input.mediaProvenance||[]).filter(mediaRightsAllowed).map(x=>x.url));const requested=[input.imageUrl,...(input.imageUrls||[])].filter((x):x is string=>Boolean(x));const manualUrls=(strictCopyright?requested.filter(x=>permitted.has(x)):requested),urls=[...manualUrls,...discovered.filter(x=>!manualUrls.includes(x))].filter((x,i,a)=>a.indexOf(x)===i).slice(0,process.env.LOW_MEMORY_MODE==='false'?14:10),downloaded:{path:string;url:string;manual:boolean;kind:'image'|'video';sceneIndex?:number;sortOrder?:number}[]=[];for(let i=0;i<urls.length;i++){await waitWhilePaused();const url=urls[i],prov=provenanceByUrl.get(url),kind=prov?.kind==='video'?'video':'image';checkpoint(job,`downloading-${kind}-${i+1}`);try{const path=kind==='video'?await downloadRemoteVideo(url,`${base}-video-${i+1}`):await downloadRemoteImage(url,`${base}-image-${i+1}`);downloaded.push({path,url,manual:manualUrls.includes(url),kind,sceneIndex:prov?.sceneIndex,sortOrder:prov?.sortOrder})}catch(e){console.warn(`${kind==='video'?'Video':'Image'} ${i+1} download skipped:`,e)}job.progress=Math.min(36,20+Math.round(((i+1)/Math.max(1,urls.length))*16));save(job)}
checkpoint(job,'selecting-media');const imageDownloads=downloaded.filter(x=>x.kind==='image').map(x=>({path:x.path,url:x.url,manual:x.manual})),media=await selectBestMedia(imageDownloads,process.env.LOW_MEMORY_MODE==='false'?10:8);for(const r of media.rejected)console.warn(`Smart Media rejected ${r.url||r.path}: ${r.width}x${r.height} — ${r.reason}`);const acceptedImages=new Set(media.selected.map(x=>x.url));const remoteItems=downloaded.filter(x=>x.kind==='video'||acceptedImages.has(x.url)).sort((a,b)=>(a.sortOrder??99)-(b.sortOrder??99)).map(x=>({path:x.path,kind:x.kind,sourceCredit:mediaCredit(x.url,input.mediaProvenance),sceneIndex:x.sceneIndex,url:x.url}));let mediaItems:RenderMediaItem[]=[...(input.localMedia||[]),...remoteItems].slice(0,10);let scenes:VideoScene[]|undefined;
job.progress=42;checkpoint(job,'tts');await waitWhilePaused();await withTimeout(generateSpeech({text:input.text,voice:input.voice??'vieneu-minh-duc',rate:input.voiceRate??'+0%',style:input.voiceStyle??(input.breaking?'breaking':'news'),audioPath:`${base}.mp3`,srtPath:`${base}.srt`,ownerId:input.ownerId}),TTS_TIMEOUT_MS,'TTS');const audioDuration=Math.max(1,Math.min(600,await mediaDurationSeconds(`${base}.mp3`)));
if(input.renderMode==='audio'){
  job.output=`${base}.mp3`;
  job.progress=100;job.status='ready';job.error=undefined;job.nextAttemptAt=undefined;job.interruptedAt=undefined;completedSinceStart++;
  checkpoint(job,'completed-audio');
  await cleanupOldOutput([job.id]).catch(()=>undefined);
  return;
}
checkpoint(job,'scene-sync');if(input.smartScenes!==false){const valid=(input.scenes||[]).filter(s=>s.imageIndex>=0&&s.imageIndex<mediaItems.length&&s.startRatio>=0&&s.endRatio<=1&&s.endRatio>s.startRatio);if(!valid.length){const slotPlan=directScenes(input.text,6,6),localItems=[...(input.localMedia||[])].sort((a,b)=>(a.kind==='video'?0:1)-(b.kind==='video'?0:1)),localByScene=new Map<number,RenderMediaItem>(),unusedLocal:RenderMediaItem[]=[];for(const item of localItems){if(Number.isInteger(item.sceneIndex)&&!localByScene.has(item.sceneIndex!))localByScene.set(item.sceneIndex!,item);else unusedLocal.push(item)}const byScene=new Map<number,typeof remoteItems[number]>(),unusedRemote=[...remoteItems];for(const item of remoteItems)if(Number.isInteger(item.sceneIndex)&&!byScene.has(item.sceneIndex!))byScene.set(item.sceneIndex!,item);const arranged:RenderMediaItem[]=[];for(let i=0;i<slotPlan.length;i++){let item=localByScene.get(i);if(!item){const exact=byScene.get(i);if(exact){item=exact;const pos=unusedRemote.indexOf(exact);if(pos>=0)unusedRemote.splice(pos,1)}}if(!item)item=unusedLocal.shift();if(!item)item=unusedRemote.shift();if(!item)item={path:await createSceneVisualCard(base,i,slotPlan[i].text,input.headline),kind:'image',sourceCredit:'Minh họa nguyên bản',sceneIndex:i};arranged.push(item)}mediaItems=arranged.length?arranged:mediaItems}if(mediaItems.length){scenes=(valid.length?valid:(input.shotCraft===false?directScenes(input.text,mediaItems.length):craftShotPlan({text:input.text,imageCount:mediaItems.length,format:input.breaking?'breaking':'latest',durationSeconds:audioDuration}).scenes)).map(s=>({...s,sourceCredit:(s as VideoScene).sourceCredit||mediaItems[s.imageIndex]?.sourceCredit}))}}job.progress=60;checkpoint(job,'pre-ffmpeg-check');await waitWhilePaused();await waitForRenderResources();await ensureStorageForRender([job.id]);checkpoint(job,'ffmpeg');
if(input.renderMode==='landscape'){
  job.output=await renderStudioLandscapeVideo({
    audioPath:`${base}.mp3`,srtPath:`${base}.srt`,outputPath:`${base}.mp4`,
    duration:audioDuration,headline:input.headline,channelName:input.channelName,mediaItems,scenes
  });
  await writeShotCraftRenderManifest({outputPath:job.output,headline:input.headline,template:input.template,motion:input.motion,scenes:scenes||[],aspectRatio:'16:9',width:1920,height:1080});
}else{
  job.output=await renderNewsVideo({audioPath:`${base}.mp3`,srtPath:`${base}.srt`,outputPath:`${base}.mp4`,duration:audioDuration,headline:input.headline,publishedAt,breaking:input.breaking,mediaItems,scenes,template:input.template,motion:input.motion,tickerMode:input.tickerMode,tickerText:input.tickerText,tickerSpeed:input.tickerSpeed,channelName:input.channelName});
  await writeShotCraftRenderManifest({outputPath:job.output,headline:input.headline,template:input.template,motion:input.motion,scenes:scenes||[]});
}job.progress=100;job.status='ready';job.error=undefined;job.nextAttemptAt=undefined;job.interruptedAt=undefined;completedSinceStart++;checkpoint(job,'completed');await cleanupOldOutput([job.id]).catch(()=>undefined)
